...
export const Route = createFileRoute('/puma/revenue-cvm-outlet')({
...
function RouteComponent() {
    ...
    const { data, refetch, isFetching, error, isError } = useQuery({
        queryKey: ['revenue-cvm-outlet', date, branch, subbranch, cluster, kabupaten],
        queryFn: async ({ queryKey }) => {
            const [_key, dateKey, branchKey, subbranchKey, clusterKey, kabupatenKey] = queryKey

            const response = await client.api['revenue-cvm-outlet-v2'].$get({
                query: {
    ...
    return (
        <div className="px-4 lg:px-6">
            <div className="overflow-hidden min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 space-y-4">
                ...
                <DataTable data={data} refetch={refetch} latestUpdatedData={daysBehind} title="Revenue CVM Outlet" date={date} isLoading={isFetching || isLoading} />
            </div>
        </div>
    )
}
