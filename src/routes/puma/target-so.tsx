...
export const Route = createFileRoute('/puma/target-so')({
...
function RouteComponent() {
    ...
    const defaultDate = React.useMemo(() => {
        if (maxDateData?.tgl_so) {
            setStates({ date: maxDateData.tgl_so })
            return maxDateData.tgl_so
        }
    ...
    const { data, refetch, isFetching, error, isError } = useQuery({
        queryKey: ['target-so', date, branch, subbranch, cluster, kabupaten],
        queryFn: async ({ queryKey }) => {
            const [_key, dateKey, branchKey, subbranchKey, clusterKey, kabupatenKey] = queryKey

            const response = await client.api['target-so'].$get({
                query: {
    ...
    return (
        <div className="px-4 lg:px-6">
            <div className="overflow-hidden min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 space-y-4">
                ...
                <DataTable data={data} refetch={refetch} latestUpdatedData={daysBehind} title="Target So" date={date} isLoading={isFetching || isLoading} />
            </div>
        </div>
    )
}
