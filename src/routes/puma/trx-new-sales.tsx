...
export const Route = createFileRoute('/puma/trx-new-sales')({
...
function RouteComponent() {
    ...
    const defaultDate = React.useMemo(() => {
        if (maxDateData?.tgl_ns) {
            setStates({ date: maxDateData.tgl_ns })
            return maxDateData.tgl_ns
        }
    ...
    const { data, refetch, isFetching, error, isError } = useQuery({
        queryKey: ['trx-new-sales', date, branch, subbranch, cluster, kabupaten],
        queryFn: async ({ queryKey }) => {
            const [_key, dateKey, branchKey, subbranchKey, clusterKey, kabupatenKey] = queryKey

            const response = await client.api['trx-new-sales'].$get({
                query: {
    ...
    return (
        <div className="px-4 lg:px-6">
            <div className="overflow-hidden min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 space-y-4">
                ...
                <DataTable data={data} refetch={refetch} latestUpdatedData={daysBehind} title="Trx New Sales All" date={date} isLoading={isFetching} />
            </div>
        </div>
    )
}
