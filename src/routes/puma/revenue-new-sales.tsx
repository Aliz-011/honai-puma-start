...
export const Route = createFileRoute('/puma/revenue-new-sales')({
...
function RouteComponent() {
    ...
    const { data, refetch, isFetching, error, isError } = useQuery({
        ...
        queryFn: async ({ queryKey }) => {
            ...
            const response = await client.api['revenue-new-sales-v2'].$get({
                query: {
    ...
    return (
        <div className="px-4 lg:px-6">
            <div className="overflow-hidden min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 space-y-4">
                ...
                <DataTable data={data} refetch={refetch} latestUpdatedData={daysBehind} title="Revenue New Sales" date={date} isLoading={isFetching || isLoading} />
            </div>
        </div>
    )
}
