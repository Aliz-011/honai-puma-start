...
export const Route = createFileRoute('/puma/revenue-redeem-pv')({
...
function RouteComponent() {
    ...
    const { data, refetch, isFetching, error, isError } = useQuery({
        queryKey: ['revenue-redeem-pv', date, branch, subbranch, cluster, kabupaten],
        queryFn: async ({ queryKey }) => {
            const [_key, dateKey, branchKey, subbranchKey, clusterKey, kabupatenKey] = queryKey

            const response = await client.api['revenue-redeem-pv'].$get({
                query: {
    ...
    return (
        <div className="px-4 lg:px-6">
            <div className="overflow-hidden min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 space-y-4">
                <Filters daysBehind={3} />
                <DataTable data={data} refetch={refetch} latestUpdatedData={3} title="Revenue Redeem PV" date={date} isLoading={isFetching} />
            </div>
        </div>
    )
}
