...
const searchParams = {
    date: parseAsString.withDefault(format(subDays(new Date(), 3), 'yyyy-MM-dd')),
...
export const Route = createFileRoute('/puma/revenue-cvm')({
...
function RouteComponent() {
    ...
    const defaultDate = React.useMemo(() => {
        if (maxDateData?.tgl_cvm) {
            setStates({ date: maxDateData.tgl_cvm })
            return maxDateData.tgl_cvm
        }
    ...
    const { data, refetch, isFetching, error, isError } = useQuery({
        ...
        queryFn: async ({ queryKey }) => {
            ...
            const response = await client.api['revenue-cvm-v2'].$get({
                query: {
    ...
    return (
        <div className="px-4 lg:px-6">
            <div className="overflow-hidden min-h-screen rounded-2xl bg-white px-2 dark:border-gray-800 dark:bg-white/3 space-y-4">
                <Filters daysBehind={daysBehind} isLoading={isLoading} />
                <DataTable data={data} refetch={refetch} latestUpdatedData={daysBehind} title="Revenue CVM All" date={date} isLoading={isFetching || isLoading} />
            </div>
        </div>
    )
}
